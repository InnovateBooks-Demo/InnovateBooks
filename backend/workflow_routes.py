...
class RevenueLeadUpdate(BaseModel):
    # everything optional (so PUT doesn't wipe fields)
    ...
    # qualification
    problem_identified: Optional[bool] = None
    budget_mentioned: Optional[str] = None
    authority_known: Optional[bool] = None
    need_timeline: Optional[bool] = None

    # notes/action
    notes: Optional[str] = None
    next_action: Optional[str] = None

    # flat address support (if your UI sends these)
...
async def create_revenue_lead(lead: RevenueLeadCreate, db = Depends(get_db)):
    ...
    # ✅ Normalize deal values (support frontend sending any of these)
    if data.get("expected_deal_value") is not None:
        data["estimated_deal_value"] = data.get("expected_deal_value")
    if data.get("deal_value") is not None:
        data["estimated_deal_value"] = data.get("deal_value")

    # ✅ If frontend ever sends contacts/address/business_profile (extra keys),
    # keep the important "flat" fields in sync (detail page uses these)
    contacts = data.get("contacts") or []
    if isinstance(contacts, list) and len(contacts) > 0:
        primary = next(
            (c for c in contacts if isinstance(c, dict) and c.get("is_primary")),
            None,
        )
        if not primary and isinstance(contacts[0], dict):
            primary = contacts[0]

        if primary:
            data["contact_name"] = primary.get("full_name") or data.get("contact_name")
            data["contact_email"] = primary.get("email") or data.get("contact_email")
            data["contact_phone"] = primary.get("phone") or data.get("contact_phone")

    addr = data.get("address") or {}
    if isinstance(addr, dict):
        # keep flat keys too (some UI / old code reads flat fields)
        if addr.get("country"):
            data["country"] = addr["country"]
        for k in ("street", "city", "state", "postal_code"):
    ...
    data["created_at"] = datetime.now(timezone.utc).isoformat()
    data["updated_at"] = data["created_at"]
    data["next_action"] = data.get("next_action") or "Initial contact"

    await db.revenue_workflow_leads.insert_one(data)

    # Create workspace task for follow-up
    await create_workspace_task(
        db,
        {
            "context_id": data["lead_id"],
            "task_type": "action",
            "title": f"Initial Contact: {data.get('company_name')}",
            "description": (
                f"Reach out to {data.get('contact_name')} at {data.get('company_name')} "
                f"- Estimated value: ₹{data.get('estimated_deal_value', 0):,.0f}"
            ),
            "assigned_to_user": data.get("owner_id"),
            "priority": "high"
            if (data.get("estimated_deal_value", 0) or 0) > 1000000
            else "medium",
            "source": "revenue_workflow",
            "workflow_ref": {"type": "lead", "id": data["lead_id"], "stage": "new"},
        },
    ...
    # Create intelligence signal for new lead
    await create_intelligence_signal(
        db,
        {
            "type": "opportunity",
            "category": "revenue",
            "title": f"New Lead: {data.get('company_name')}",
            "message": (
                f"New revenue opportunity from {data.get('lead_source')} - "
                f"Est. value ₹{data.get('estimated_deal_value', 0):,.0f}"
            ),
            "severity": "low",
            "source": "workflow_engine",
            "context_type": "revenue_lead",
            ...
            "metadata": {
                ...
                "value": data.get("estimated_deal_value"),
            },
        },
    )
    ...
    await create_activity_record(
        db,
        {
            ...
            "entity_name": data.get("company_name"),
            "description": (
                f"New lead created: {data.get('company_name')} - "
                f"₹{data.get('estimated_deal_value', 0):,.0f}"
            ),
            "module": "commerce",
        },
    )

    # ✅ Return full lead (useful for frontend to instantly show)
    created = await db.revenue_workflow_leads.find_one({"lead_id": data["lead_id"]}, {"_id": 0})
    return {"success": True, "message": "Lead created", "lead_id": data["lead_id"], "lead": created}
...
@router.put("/revenue/leads/{lead_id}")
async def update_revenue_lead(lead_id: str, lead: RevenueLeadUpdate, db = Depends(get_db)):
    existing = await db.revenue_workflow_leads.find_one({"lead_id": lead_id})
    if not existing:
        raise HTTPException(status_code=404, detail="Lead not found")

    data = lead.dict(exclude_none=True)

    # ✅ unify deal values into estimated_deal_value (your DB uses this field)
    if "expected_deal_value" in data and data["expected_deal_value"] is not None:
        data["estimated_deal_value"] = data["expected_deal_value"]
    if "deal_value" in data and data["deal_value"] is not None:
        data["estimated_deal_value"] = data["deal_value"]

    # ✅ if nested address came, also keep flat fields in sync (optional)
    if "address" in data and isinstance(data["address"], dict):
        addr = data["address"]
        for k in ("country", "street", "city", "state", "postal_code"):
            if k in addr and addr[k] is not None:
                data[k] = addr[k]
        if "country" in addr and addr.get("country"):
            data["country"] = addr["country"]

    # ✅ if contacts exist, set flat primary contact fields (detail page uses these)
    if "contacts" in data and isinstance(data["contacts"], list) and len(data["contacts"]) > 0:
        ...
        for c in data["contacts"]:
            if isinstance(c, dict) and c.get("is_primary"):
                ...
                break
        if not primary:
            primary = data["contacts"][0] if isinstance(data["contacts"][0], dict) else None

        if primary:
            data["contact_name"] = primary.get("full_name") or existing.get("contact_name")
            data["contact_email"] = primary.get("email") or existing.get("contact_email")
            data["contact_phone"] = primary.get("phone") or existing.get("contact_phone")
    ...
    # ✅ return updated document (helps frontend instantly)
    updated = await db.revenue_workflow_leads.find_one({"lead_id": lead_id}, {"_id": 0})
    return {"success": True, "message": "Lead updated", "lead": updated}
